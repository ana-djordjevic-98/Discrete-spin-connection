...
for j=1:1:p
    if j==1
        ...
        l(:,:,2*p)=-Omega(:,:,p)*(-Vgamma(1,j)*gamma_x-Vgamma(2,j)*gamma_y)*Omega(:,:,p)';  
    end
    if j>=2
        l(:,:,j-1)=Vgamma1(1,j-1)*gamma_x+Vgamma1(2,j-1)*gamma_y;
        l(:,:,j+p-1)=-Omega(:,:,j-1)*(-Vgamma(1,j)*gamma_x-Vgamma(2,j)*gamma_y)*Omega(:,:,j-1)';
    end
end
...
for j=1:1:p
    for m=1:1:p
        ...
        if j>=2
            ...
            figure (1)
            plot(xc(j-1,m)+Rc(j-1)*cos(t),yc(j-1,m)+Rc(j-1)*sin(t),'k');
        else
            if rx(p,m)>=xc(p,m)
            ...
            figure (1)
            plot(xc(p,m)+Rc(p)*cos(t),yc(p,m)+Rc(p)*sin(t),'k');
        end    
    end
end        
